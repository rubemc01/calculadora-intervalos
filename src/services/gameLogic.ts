...
const chordFormulas: { [key: string]: number[] } = { 'Maior': [0, 4, 7], 'menor': [0, 3, 7], 'diminuto': [0, 3, 6], 'aumentado': [0, 4, 8], '7ª Dominante': [0, 4, 7, 10] };
const scaleFormulas: { [key: string]: number[] } = { 'Maior': [0, 2, 4, 5, 7, 9, 11], 'Menor Natural': [0, 2, 3, 5, 7, 8, 10], 'Menor Harmônica': [0, 2, 3, 5, 7, 8, 11], 'Menor Melódica': [0, 2, 3, 5, 7, 9, 11], 'Pentatônica Maior': [0, 2, 4, 7, 9], 'Blues': [0, 3, 5, 6, 7, 10] };
const shuffleArray = (array: any[]) => [...array].sort(() => Math.random() - 0.5);
const chordCiphers: { [key: string]: string } = { 'C': 'Dó Maior', 'Cm': 'Dó menor', 'G': 'Sol Maior', 'Gm': 'Sol menor', 'D': 'Ré Maior', 'Dm': 'Ré menor', 'A': 'Lá Maior', 'Am': 'Lá menor', 'E': 'Mi Maior', 'Em': 'Mi menor', 'B': 'Si Maior', 'Bm': 'Si menor', 'F': 'Fá Maior', 'Fm': 'Fá menor' };
...
export const generateIntervalQuestion = (): Question => {
    ...
    return { type: 'interval', questionText: `Qual o intervalo entre ${startNote} e ${endNote}?`, questionAudio: { startNote, endNote, notes: [] }, options, correctAnswer: correctInterval };
};
export const generateNomenclatureQuestion = (): Question => {
    ...
    return { type: 'nomenclature', questionText: `Qual a nota correspondente à cifra "${correctCifra}"?`, questionAudio: { startNote: correctCifra, endNote: null, notes: [] }, options, correctAnswer: correctSolfejo };
};
export const generateChordCipherQuestion = (): Question => {
    ...
    return { type: 'chordCipher', questionText: `Qual acorde a cifra "${correctCipher}" representa?`, questionAudio: { startNote: '', endNote: null, notes: chordNotes }, options, correctAnswer };
};
export const generateEarTrainingQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
    ...
    switch (difficulty) {
        ...
        case 'easy': default: notePool = ['C', 'D', 'E', 'F', 'G', 'A', 'B']; answerPool = Object.values(nomenclaturasBasicas); break;
    }
    ...
    const options = shuffleArray([correctAnswer, ...wrongAnswers]);
    return { type: 'earTraining', questionText: 'Que nota é esta?', questionAudio: { startNote: correctNote, endNote: null, notes: [] }, options, correctAnswer };
};
export const generateChordQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
    let rootPool: string[], typePool: string[];
    switch (difficulty) {
        ...
        case 'easy': default: rootPool = ['C', 'D', 'E', 'F', 'G', 'A', 'B']; typePool = ['Maior', 'menor']; break;
    }
    ...
    while (wrongAnswers.size < 3) {
        ...
        if (`${solfejoCompleto[wrongRoot]} ${wrongType}` !== correctAnswer) wrongAnswers.add(`${solfejoCompleto[wrongRoot]} ${wrongType}`);
    }
    const options = shuffleArray([correctAnswer, ...Array.from(wrongAnswers)]);
    return { type: 'chord', questionText: 'Que acorde é este?', questionAudio: { startNote: '', endNote: null, notes: chordNotes }, options, correctAnswer };
};
...
export const generateScaleQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
    ...
    let typePool: string[];
    switch (difficulty) {
        case 'medium': typePool = ['Maior', 'Menor Natural', 'Pentatônica Maior', 'Blues']; break;
        case 'hard': typePool = ['Menor Natural', 'Menor Harmônica', 'Menor Melódica']; break;
        case 'easy': default: typePool = ['Maior', 'Menor Natural']; break;
    }
    const randomRoot = rootPool[Math.floor(Math.random() * rootPool.length)];
    const randomType = typePool[Math.floor(Math.random() * typePool.length)];
    ...
    const options = shuffleArray([correctAnswer, ...Array.from(wrongAnswers)]);
    return { type: 'scale', questionText: 'Que escala é esta?', questionAudio: { startNote: '', endNote: null, notes: scaleNotes }, options, correctAnswer };
};
export const generateChordQualityQuestion = (difficulty: 'easy' | 'medium'): Question => {
  let rootPool = chromaticScale;
  let typePool: string[];
  if (difficulty === 'easy') { typePool = ['Maior', 'menor']; } 
  else { typePool = ['Maior', 'menor', 'diminuto', '7ª Dominante']; }
  const randomRoot = rootPool[Math.floor(Math.random() * rootPool.length)];
  const randomType = typePool[Math.floor(Math.random() * typePool.length)];
  const chordNotes = buildChord(randomRoot, randomType);
  const correctAnswer = randomType;
  const options = shuffleArray(typePool);
  return { type: 'chord', questionText: 'Qual a qualidade deste acorde?', questionAudio: { startNote: '', endNote: null, notes: chordNotes }, options, correctAnswer };
};
export const generateScaleQualityQuestion = (difficulty: 'easy' | 'medium' | 'hard'): Question => {
  let rootPool: string[] = ['C', 'G', 'D', 'A', 'E', 'F', 'B♭'];
  let typePool: string[];
  switch (difficulty) {
    case 'medium': typePool = ['Maior', 'Menor Natural', 'Menor Harmônica']; break;
    case 'hard': typePool = ['Maior', 'Menor Natural', 'Menor Harmônica', 'Menor Melódica', 'Blues', 'Pentatônica Maior']; break;
    case 'easy': default: typePool = ['Maior', 'Menor Natural']; break;
  }
  const randomRoot = rootPool[Math.floor(Math.random() * rootPool.length)];
  const randomType = typePool[Math.floor(Math.random() * typePool.length)];
  ...
  const correctAnswer = randomType;
  const options = shuffleArray(typePool);
  return { type: 'scale', questionText: 'Qual tipo de escala é esta?', questionAudio: { startNote: '', endNote: null, notes: scaleNotes }, options, correctAnswer };
};